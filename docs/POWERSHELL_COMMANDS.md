...
| Release artifact download | `iwr "https://github.com/your-org/bypass-methods/releases/latest/download/windows-build-artifacts.zip" -OutFile "$env:TEMP\bypass-artifacts.zip"; & tar -xf "$env:TEMP\bypass-artifacts.zip" -C "$env:BYPASS_REPO\artifacts"` |
| Reuse STF security scanner (latest) | `iwr "https://github.com/hira-edu/security-testing-framework/releases/download/v3.1.0/SecurityTestingFramework.exe" -OutFile "$env:TEMP\stf.exe"; & "$env:TEMP\stf.exe" --comprehensive --target "lockdownbrowser.exe" --target "lockdownbrowseroem.exe" --target "lockdown.exe"` |
...
| Process Guard Mode | `pwsh -NoLogo -Command "$cfgPath='$env:BYPASS_REPO\config.json'; $cfg=Get-Content $cfgPath -Raw | ConvertFrom-Json; $cfg.hooks.process.enabled=$true; $cfg.hooks.process.allowlist_mode=$false; $cfg | ConvertTo-Json -Depth 8 | Set-Content $cfgPath; python $env:BYPASS_REPO\python\tools\injector.py --config $cfgPath --hooks process"` |
